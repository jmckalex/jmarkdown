...
- **Nesting:** arbitrarily deep. Same-name nesting is depth-counted in the tokenizer; differently-named nesting is free (inner block is re-lexed as markdown). **Nested blocks may be indented** for readability — each block dedents its body (strips the common leading whitespace) before processing, so top-level blocks sit at column 0 while nested ones can be indented (keep a block's opener/closer aligned; relative indentation, e.g. inside a code fence, is preserved). An orphan opener (no `@end`) is emitted literally with a stderr warning — it never swallows the document or throws.
